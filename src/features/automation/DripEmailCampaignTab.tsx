...
export interface EmailBlacklistEntry {
  id: string;
  email: string;
  reason: 'already_sent' | 'manual_blacklist' | 'bounced' | 'unsubscribed' | 'complaint';
  notes?: string | null;
...
export interface EmailBlacklistResponse {
  items: EmailBlacklistEntry[];
  stats: {
    total: number;
    alreadySent: number;
    manualBlacklist: number;
    bounced: number;
    unsubscribed: number;
  };
  pagination: {
...
export function DripEmailCampaignTab() {
  ...
  // Subtab Navigation
  const [activeSubTab, setActiveSubTab] = useState<'campaigns' | 'blacklist'>('campaigns');

  // Blacklist & Suppression State
  const [blacklistItems, setBlacklistItems] = useState<EmailBlacklistEntry[]>([]);
  ...
  const [blacklistReasonFilter, setBlacklistReasonFilter] = useState<string>('all');
  const [blacklistPage, setBlacklistPage] = useState(1);
  const [blacklistLimit, setBlacklistLimit] = useState(20);
  const [blacklistTotal, setBlacklistTotal] = useState(0);
  const [blacklistTotalPages, setBlacklistTotalPages] = useState(1);

  // Manual Blacklist Modal State
  const [manualBlacklistModalOpen, setManualBlacklistModalOpen] = useState(false);
  const [manualEmail, setManualEmail] = useState('');
  const [manualReason, setManualReason] = useState<'manual_blacklist' | 'complaint' | 'bounced' | 'unsubscribed'>('manual_blacklist');
...
<p style="margin-top: 24px;"><em>Wassalamu'alaikum Warahmatullahi Wabarakatuh.</em><br><strong style="color: #14352A;">Tim Layanan Jamaah &amp; Hubungan Umat<br>Yayasan Tarbiyah Sunnah Bandung</strong></p>
  ...
  const fetchBlacklist = async () => {
    try {
      ...
      const params = new URLSearchParams({
        page: String(blacklistPage),
        limit: String(blacklistLimit),
        search: blacklistSearch.trim(),
        reason: blacklistReasonFilter,
      });
      const res = await apiClient<EmailBlacklistResponse>(`/automation/email-blacklist?${params.toString()}`);
      if (res.data) {
        setBlacklistItems(res.data.items || []);
        setBlacklistStats(res.data.stats || null);
        setBlacklistTotal(res.data.pagination?.total ?? 0);
        setBlacklistTotalPages(res.data.pagination?.totalPages ?? 1);
      }
    } catch (err: any) {
      console.warn('[Fetch Blacklist Err]:', err);
    } finally {
  ...
  }, [activeSubTab, blacklistPage, blacklistLimit, blacklistReasonFilter]);
  ...
  const handleDispatchToday = async () => {
    ...
    setConfirmDialog({
      ...
      message: (
        <div>
          ...
          <p className="text-xs text-[#6B7A72] mt-2">
            Catatan interaksi CRM akan terisi otomatis untuk setiap email yang berhasil diterima. Email yang terdaftar di blacklist akan dilewati otomatis tanpa memotong kuota harian.
          </p>
        </div>
      ),
      ...
      variant: 'success',
      onConfirm: async () => {
        const preTotalSent = currentCampaign.stats.totalSent;
        try {
          setDispatching(true);
          ...
          const skippedCount = res.data?.skippedBlacklisted || 0;
          const failedCount = res.data?.failedCount || 0;

          let msg = `✓ Berhasil memproses pengiriman: ${successCount} email terkirim.`;
          if (skippedCount > 0) {
            msg += ` (${skippedCount} blacklist dilewati)`;
          }
          if (failedCount > 0) {
            msg += ` [${failedCount} gagal kirim]`;
          }
          ...
          // Double check if server finished in background to prevent false error notifications
          try {
            const verifyRes = await apiClient<DripEmailCampaign>(`/automation/email-campaigns/${currentCampaign.id}`);
            if (verifyRes.data && verifyRes.data.stats.totalSent > preTotalSent) {
              const diff = verifyRes.data.stats.totalSent - preTotalSent;
              showToast(`✓ Batch email telah diproses di server (${diff} email baru terkirim).`, 'success');
              await fetchCampaigns();
              fetchBlacklist();
              return;
            }
  ...
  const handleBlacklistRecipient = (r: DripRecipient) => {
    if (!currentCampaign) return;
    setConfirmDialog({
      isOpen: true,
      title: 'Tambahkan ke Blacklist Permanen',
      message: (
        <div>
          <p>
            Keluarkan <strong>{r.fullName}</strong> ({r.email}) dari seluruh pengiriman email sekarang dan di masa depan?
          </p>
          <p className="text-xs text-[#6B7A72] mt-2">
            Email ini akan berstatus <strong>Blacklist</strong> dan otomatis dilewati tanpa memakan kuota broadcast harian.
          </p>
        </div>
      ),
      confirmLabel: 'Ya, Blacklist Email Ini',
      variant: 'danger',
      onConfirm: async () => {
        try {
          setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
          await apiClient(`/automation/email-campaigns/${currentCampaign.id}/recipients/${encodeURIComponent(r.email)}/blacklist`, {
            method: 'POST',
          });
          showToast(`✓ ${r.email} berhasil dimasukkan ke daftar blacklist.`, 'success');
          await fetchCampaigns();
          fetchBlacklist();
        } catch (err: any) {
          showToast(err.message || 'Gagal menambahkan ke blacklist', 'error');
        }
      },
    });
  };

  const handleUnblacklistRecipient = async (r: DripRecipient) => {
    if (!currentCampaign) return;
    try {
      await apiClient(`/automation/email-campaigns/${currentCampaign.id}/recipients/${encodeURIComponent(r.email)}/unblacklist`, {
        method: 'POST',
      });
      showToast(`✓ ${r.email} dipulihkan kembali ke antrean pending.`, 'success');
      await fetchCampaigns();
      fetchBlacklist();
    } catch (err: any) {
      showToast(err.message || 'Gagal memulihkan penerima', 'error');
    }
  };
  ...
  const handleAddManualBlacklist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient('/automation/email-blacklist', {
        method: 'POST',
        body: JSON.stringify({
          email: cleanEmail,
          reason: manualReason,
      ...
      showToast(`✓ Alamat email ${cleanEmail} berhasil dimasukkan ke blacklist.`, 'success');
      setManualBlacklistModalOpen(false);
      setManualEmail('');
      setManualNotes('');
      fetchBlacklist();
      fetchCampaigns();
  ...
  const handleRemoveFromBlacklist = (entry: EmailBlacklistEntry) => {
    setConfirmDialog({
      isOpen: true,
      title: 'Hapus dari Daftar Blacklist',
      message: (
        <div>
          <p>
            Buka blokir untuk <strong>{entry.email}</strong>?
          </p>
          <p className="text-xs text-[#6B7A72] mt-2">
            Email ini akan kembali memenuhi syarat untuk menerima email kampanye di masa depan jika masuk dalam target.
          </p>
        </div>
      ),
      confirmLabel: 'Buka Blokir / Hapus',
      variant: 'warning',
      onConfirm: async () => {
        try {
          setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
          await apiClient(`/automation/email-blacklist/${entry.id}`, {
            method: 'DELETE',
          });
          showToast(`✓ Blokir ${entry.email} berhasil dicabut.`, 'success');
          fetchBlacklist();
          fetchCampaigns();
        } catch (err: any) {
          showToast(err.message || 'Gagal menghapus dari blacklist', 'error');
        }
      },
    });
  };

  const handleExportBlacklistCsv = () => {
    if (blacklistItems.length === 0) {
      showToast('Tidak ada data blacklist untuk diekspor', 'warning');
      return;
    }
    const headers = ['Alamat Email', 'Kategori Alasan', 'Catatan / Program', 'Tanggal Masuk'];
    const rows = blacklistItems.map((item) => [
      `"${item.email}"`,
      `"${item.reason}"`,
    ...
    link.setAttribute('download', `email-blacklist-suppression-${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast('File CSV blacklist berhasil diunduh.', 'success');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sub-Tab Navigation */}
      <div className="flex items-center gap-2 p-1.5 bg-[#F2EEE4] rounded-2xl border border-[#1B4332]/12 w-fit">
        <button
          type="button"
        ...
        >
          <Mail className="w-3.5 h-3.5 text-[#E0B970]" />
          <span>Program Kampanye &amp; Antrean</span>
        ...
        >
          <ShieldAlert className="w-3.5 h-3.5 text-[#E0B970]" />
          <span>Email Blacklist &amp; Supresi Permanen</span>
          {blacklistStats && (
            <span className="px-1.5 py-0.5 rounded-full text-[10px] font-mono bg-white/20">
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-[#1B4332]/12 bg-[#FBF9F4] text-[#14352A] text-[10.5px] font-mono font-bold uppercase tracking-wider">
                      ...
                      <th className="py-3 px-3 text-center">Aksi / Supresi</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={r.personId || idx} className="hover:bg-[#FBF9F4] transition-colors">
                          ...
                          <td className="py-3 px-3 whitespace-nowrap">
                            {r.status === 'sent' ? (
                              ...
                              <span
                                className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-100 text-slate-700 border border-slate-300 inline-flex items-center gap-1"
                                title={r.error || 'Terdaftar di Blacklist / Sudah terkirim sebelumnya'}
                              >
                                <Ban className="w-3 h-3 text-slate-500" /> Blacklist (Skip)
                              </span>
                            ) : r.status === 'failed' ? (
                              <span
                          ...
                          <td className="py-3 px-3 text-center whitespace-nowrap">
                            {r.status === 'blacklisted' ? (
                              <button
                                type="button"
                                onClick={() => handleUnblacklistRecipient(r)}
                                className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-[#2F7D4F] border border-emerald-200 rounded-lg text-[10.5px] font-bold inline-flex items-center gap-1 shadow-2xs transition-all active:scale-95"
                                title="Pulihkan email ini ke antrean pengiriman"
                              >
                              ...
                              <span
                                className="text-[10px] font-mono text-emerald-700 font-semibold inline-flex items-center gap-1"
                                title="Email telah terkirim dan otomatis disupresi untuk masa depan"
                              >
                                <ShieldCheck className="w-3 h-3" /> Terkirim (Skip)
                              </span>
                            ) : (
                              <button
                                type="button"
                                onClick={() => handleBlacklistRecipient(r)}
                                className="px-2 py-1 bg-[#FBF9F4] hover:bg-rose-50 text-slate-600 hover:text-rose-700 border border-[#1B4332]/14 hover:border-rose-300 rounded-lg text-[10.5px] font-bold inline-flex items-center gap-1 shadow-2xs transition-all active:scale-95"
                                title="Keluarkan &amp; skip email ini dari seluruh pengiriman selamanya"
                              >
      ...
      {/* BLACKLIST & SUPPRESSION VIEW */}
      {activeSubTab === 'blacklist' && (
        <div className="space-y-5 animate-in fade-in duration-150">
          {/* Blacklist Info Banner */}
          <div className="p-4 bg-[#FBF9F4] rounded-2xl border border-[#1B4332]/15 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold font-display text-sm text-[#1C2321]">
                  Pencegahan Duplikasi &amp; Supresi Email Global
                </h3>
                <p className="text-[#6B7A72] text-xs mt-0.5">
                  Setiap email yang berhasil terkirim akan otomatis masuk ke daftar ini agar <strong>tidak pernah dikirimkan kembali (auto-skip)</strong> pada hari berikutnya maupun program kampanye lain. Admin/Amil juga dapat menambahkan blacklist manual untuk email komplain atau mental.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-3 py-2 bg-white hover:bg-[#F2EEE4] text-[#14352A] rounded-xl border border-[#1B4332]/14 text-xs font-bold transition-all shadow-2xs flex items-center gap-1.5 active:scale-98"
              >
                <Download className="w-3.5 h-3.5 text-[#14352A]" />
                <span>Ekspor Blacklist</span>
              </button>
              <button
                type="button"
                onClick={() => setManualBlacklistModalOpen(true)}
                className="px-3.5 py-2 bg-[#1B4332] hover:bg-[#14352A] text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 active:scale-98"
              >
          ...
          {/* 5 KPI Cards for Blacklist */}
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div className="p-4 bg-white rounded-xl border border-[#1B4332]/12 shadow-2xs border-l-[3px] border-l-[#14352A] space-y-1">
              <span className="text-[10px] font-mono font-bold uppercase text-[#14352A]">
                TOTAL BLACKLIST
              </span>
              <div className="text-2xl font-bold font-display text-[#1C2321]">
                {blacklistStats?.total.toLocaleString('id-ID') ?? 0}
              </div>
              <div className="text-[11px] text-[#6B7A72]">Dilewati dari pengiriman</div>
            </div>

            <div className="p-4 bg-white rounded-xl border border-[#1B4332]/12 shadow-2xs border-l-[3px] border-l-[#2F7D4F] space-y-1">
              <span className="text-[10px] font-mono font-bold uppercase text-[#2F7D4F]">
                SUDAH PERNAH DIKIRIM
              </span>
              <div className="text-2xl font-bold font-display text-[#2F7D4F]">
                {blacklistStats?.alreadySent.toLocaleString('id-ID') ?? 0}
              </div>
              <div className="text-[11px] text-[#6B7A72]">Auto-skip (tidak duplikat)</div>
            </div>

            <div className="p-4 bg-white rounded-xl border border-[#1B4332]/12 shadow-2xs border-l-[3px] border-l-[#C77A16] space-y-1">
              <span className="text-[10px] font-mono font-bold uppercase text-[#C77A16]">
                MANUAL AMIL / ADMIN
              </span>
              <div className="text-2xl font-bold font-display text-[#C77A16]">
                {blacklistStats?.manualBlacklist.toLocaleString('id-ID') ?? 0}
              </div>
              <div className="text-[11px] text-[#6B7A72]">Ditandai manual</div>
            </div>

            <div className="p-4 bg-white rounded-xl border border-[#1B4332]/12 shadow-2xs border-l-[3px] border-l-rose-600 space-y-1">
              ...
              <div className="text-2xl font-bold font-display text-rose-700">
                {blacklistStats?.bounced.toLocaleString('id-ID') ?? 0}
              </div>
              <div className="text-[11px] text-[#6B7A72]">Email tidak aktif/kotak penuh</div>
            </div>

            <div className="p-4 bg-white rounded-xl border border-[#1B4332]/12 shadow-2xs border-l-[3px] border-l-slate-600 space-y-1">
              ...
              <div className="text-2xl font-bold font-display text-slate-700">
                {blacklistStats?.unsubscribed.toLocaleString('id-ID') ?? 0}
              </div>
              <div className="text-[11px] text-[#6B7A72]">Berhenti berlangganan</div>
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 items-center justify-between">
              <div className="relative flex-1 w-full">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') fetchBlacklist();
                  }}
                  placeholder="Cari alamat email atau catatan blacklist..."
                  className="w-full pl-10 pr-9 py-2 text-xs font-medium border border-[#1B4332]/14 rounded-xl focus:ring-2 focus:ring-[#1B4332] bg-[#FBF9F4] text-[#1C2321] placeholder-[#8A9690] outline-none"
                />
                {blacklistSearch && (
                  <button
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                <select
                  value={blacklistReasonFilter}
                  onChange={(e) => {
                    setBlacklistReasonFilter(e.target.value);
                    setBlacklistPage(1);
                  }}
                  className="px-3 py-2 text-xs font-bold rounded-xl border border-[#1B4332]/14 bg-[#FBF9F4] text-[#14352A] outline-none"
                >
                  <option value="all">Semua Kategori Alasan</option>
                  <option value="already_sent">Sudah Pernah Terkirim (already_sent)</option>
                  <option value="manual_blacklist">Blacklist Manual Amil</option>
                  <option value="bounced">Mental / Bounced</option>
                  <option value="unsubscribed">Berhenti Langganan</option>
                ...
                <button
                  ...
                  className="p-2 bg-[#FBF9F4] hover:bg-[#F2EEE4] text-[#3D4A44] rounded-xl border border-[#1B4332]/12 transition-all flex items-center gap-1 text-xs font-semibold px-3"
                  title="Segarkan Data Blacklist"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${blacklistLoading ? 'animate-spin' : ''}`} />
                  <span>Segarkan</span>
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#1B4332]/12 shadow-2xs overflow-hidden">
            {blacklistLoading ? (
              <div className="py-16">
                <LoadingState message="Memuat daftar email blacklist..." />
              </div>
            ) : blacklistItems.length === 0 ? (
              <div className="py-16 text-center text-[#6B7A72] text-xs space-y-3">
                <div className="w-12 h-12 bg-[#FBF9F4] rounded-xl flex items-center justify-center mx-auto text-[#6B7A72]">
                  <ShieldCheck className="w-6 h-6 text-[#2F7D4F]" />
                </div>
                <p className="font-bold text-sm text-[#1C2321]">Tidak ada email blacklist pada filter ini</p>
                <p className="text-[#6B7A72]">Semua email jamaah memenuhi syarat untuk pengiriman.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-[#1B4332]/12 bg-[#FBF9F4] text-[#14352A] text-[10.5px] font-mono font-bold uppercase tracking-wider">
                      <th className="py-3 px-4">Alamat Email</th>
                      <th className="py-3 px-3">Kategori Alasan</th>
                      <th className="py-3 px-4">Catatan / Asal Kampanye</th>
                      <th className="py-3 px-4 text-right">Tanggal Masuk</th>
                  ...
                  <tbody className="divide-y divide-[#1B4332]/8 font-medium text-[#1C2321]">
                    {blacklistItems.map((item) => (
                      <tr key={item.id} className="hover:bg-[#FBF9F4] transition-colors">
                        <td className="py-3 px-4 font-mono font-bold text-[#14352A]">
                          {item.email}
                        </td>
                        <td className="py-3 px-3 whitespace-nowrap">
                          {item.reason === 'already_sent' ? (
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-[#2F7D4F]/10 text-[#2F7D4F] border border-[#2F7D4F]/25 inline-flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" /> Sudah Pernah Terkirim
                            </span>
                          ) : item.reason === 'manual_blacklist' ? (
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-50 text-amber-800 border border-amber-200 inline-flex items-center gap-1">
                              <Ban className="w-3 h-3 text-amber-600" /> Manual Amil
                            </span>
                          ) : item.reason === 'bounced' ? (
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-rose-50 text-rose-700 border border-rose-200 inline-flex items-center gap-1">
                              <AlertTriangle className="w-3 h-3" /> Mental / Bounced
                            </span>
                          ) : item.reason === 'unsubscribed' ? (
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-100 text-slate-700 border border-slate-300 inline-flex items-center gap-1">
                              <UserX className="w-3 h-3" /> Berhenti Langganan
                            </span>
                          ) : (
                        ...
                        <td className="py-3 px-3 text-center whitespace-nowrap">
                          <button
                            ...
                            className="px-2.5 py-1 bg-white hover:bg-rose-50 text-rose-700 hover:text-rose-800 border border-rose-200 rounded-lg text-[10.5px] font-bold inline-flex items-center gap-1 shadow-2xs transition-all active:scale-95"
                            title="Hapus dari daftar blacklist (Buka Blokir)"
                          >
            ...
            <div className="px-4 py-3 border-t border-[#1B4332]/10 bg-[#FBF9F4] flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#6B7A72]">
              <div>
                Menampilkan <strong className="text-[#1C2321]">{blacklistItems.length}</strong> dari{' '}
                <strong className="text-[#1C2321]">{blacklistTotal.toLocaleString('id-ID')}</strong> email terblokir
              </div>
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) => {
                    setBlacklistLimit(Number(e.target.value));
                    setBlacklistPage(1);
                  }}
                  className="px-2 py-1 text-xs font-bold rounded-lg border border-[#1B4332]/14 bg-white text-[#14352A] outline-none"
                  title="Jumlah baris per halaman"
                >
                ...
                <button
                  ...
                  disabled={blacklistPage <= 1 || blacklistLoading}
                  className="py-1 px-2.5 bg-white hover:bg-[#F2EEE4] text-[#1C2321] rounded-lg border border-[#1B4332]/12 font-semibold disabled:opacity-40 flex items-center gap-1 shadow-2xs"
                >
                  <ChevronLeft className="w-3.5 h-3.5" />
                  <span>Sebelumnya</span>
                ...
                <span className="font-mono text-xs font-semibold text-[#1C2321] px-2">
                  Halaman {blacklistPage} dari {blacklistTotalPages}
                </span>
                <button
                  onClick={() => setBlacklistPage(blacklistPage + 1)}
                  disabled={blacklistPage >= blacklistTotalPages || blacklistLoading}
                  className="py-1 px-2.5 bg-white hover:bg-[#F2EEE4] text-[#1C2321] rounded-lg border border-[#1B4332]/12 font-semibold disabled:opacity-40 flex items-center gap-1 shadow-2xs"
                >
                  <span>Berikutnya</span>
                  <ChevronRight className="w-3.5 h-3.5" />
      ...
      {/* MODAL TAMBAH BLACKLIST MANUAL */}
      {manualBlacklistModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl border border-[#1B4332]/20 w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="px-5 py-4 border-b border-[#1B4332]/10 flex items-center justify-between bg-[#FBF9F4]">
              <h3 className="text-sm font-bold font-display text-[#1C2321] flex items-center gap-2">
                <Ban className="w-4 h-4 text-rose-600" />
                <span>Tambah Email ke Blacklist / Supresi</span>
              </h3>
              <button
                type="button"
                onClick={() => setManualBlacklistModalOpen(false)}
                className="p-1 rounded-lg text-[#6B7A72] hover:text-[#1C2321]"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <form onSubmit={handleAddManualBlacklist} className="p-5 space-y-4 text-xs">
              <p className="text-[#6B7A72]">
                Alamat email yang didaftarkan ke sini akan <strong>selalu dilewati (skip)</strong> dalam seluruh kampanye broadcast email di masa depan tanpa memotong kuota harian.
              </p>

              <div>
                <label className="block text-[11px] font-bold text-[#1C2321] mb-1 uppercase font-mono">
                ...
                <input
                  ...
                  placeholder="contoh: jamaah@domain.com"
                  className="w-full px-3 py-2 border border-[#1B4332]/14 rounded-xl text-xs font-mono bg-[#FBF9F4] text-[#1C2321] focus:bg-white focus:ring-2 focus:ring-[#1B4332] outline-none"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-[#1B4332]/14 rounded-xl text-xs bg-[#FBF9F4] text-[#1C2321] focus:bg-white focus:ring-2 focus:ring-[#1B4332] outline-none"
                >
                  <option value="manual_blacklist">Blacklist Manual Amil / Permintaan Khusus</option>
                  <option value="complaint">Komplain / Melaporkan Spam</option>
                  <option value="bounced">Kotak Surat Penuh / Mental (Bounced)</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Catatan alasan pemblokiran atau permintaan dari jamaah..."
                  className="w-full px-3 py-2 border border-[#1B4332]/14 rounded-xl text-xs bg-[#FBF9F4] text-[#1C2321] focus:bg-white focus:ring-2 focus:ring-[#1B4332] outline-none"
                />
              </div>
              ...
              <div className="pt-2 border-t border-[#1B4332]/10 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#1B4332] hover:bg-[#14352A] text-white rounded-xl font-bold shadow-xs flex items-center gap-1.5 active:scale-98 disabled:opacity-50"
                >
                  {submittingManual ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin text-[#E0B970]" />
                  ) : (
                    <Ban className="w-3.5 h-3.5 text-[#E0B970]" />
                  )}
                  <span>Simpan ke Blacklist</span>
                </button>
              </div>
            </form>
          </div>
